...
class Model:
				...
				for i,mesh in enumerate(self.meshList):
					for j,mat in enumerate(mesh.matList):
						#for line in lines:
							...
							if len(values)>3:
								if os.path.exists(values[2])==True:mat.diffuse=values[2];print 'mam'
								if os.path.exists(values[3])==True:mat.normal=values[3]
								if os.path.exists(values[4])==True:mat.specular=values[4]
				matFile.close()
	...
	def draw(self):
		#self.getMat()
		file=open('sesja.txt','r')
		copylines=file.readlines()
		file=open('sesja.txt','w')
